...
router.get('/api/stats', async (req, res) => {
  try {
    ...
    const totalVolumeRaw = allPayments.reduce((sum, p) => sum + BigInt(p.amount), BigInt(0));
    ...
    res.json({ totalPayments, settledPayments: settled, totalVolume, totalFees, avgSettlementTime: '<5s', activeChains: 1 });
  } catch (error) {
...
const paymentPool = new ethers.Contract(PAYMENT_POOL_ADDRESS, PaymentPoolABI_Interface, provider);
const batchSettler = new ethers.Contract(BATCH_SETTLER_ADDRESS, BatchSettlerABI_Interface, provider);

router.get('/api/contract-status', async (req, res) => {
  try {
    const [poolPaused, settlerPaused, maxBatchSize, feeBasisPoints, feeRecipient] = await Promise.all([
      paymentPool.paused(),
      batchSettler.paused(),
    ...
    res.json({
      paymentPool: { address: PAYMENT_POOL_ADDRESS, paused: poolPaused },
      batchSettler: {
...
router.get('/api/volume-chart', async (req, res) => {
  try {
    const payments = await prisma.payment.findMany({
      where: { settled: true },
    ...
    if (dates.length === 0) { res.json([]); return; }
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      cumulative += volume;
      result.push({ date: key, volume: Math.round(volume * 100) / 100, cumulative: Math.round(cumulative * 100) / 100 });
    }
    res.json(result);
  } catch (error) {
